...
def scrape_from_csv():
    ...
    data = []
    with open("mbfc_sites_all.csv", 'r', newline='', encoding="UTF-8") as csvfile:
        ...
        try:
            ...
            if num_urls > 10:
                ...
                while len(nums_checked)<1:
                    ...
                    if num not in nums_checked and length > 200:
                        print(f"{num}: {urls[num]}")
                        print(f"Word count: {length}")
                        print(questionable)
                        db_setup.insert_page(conn, name, domain, urls[num], bias, credibility, reporting, questionable, content)
